...
function howManyMovies(moviesArray) {
    //const spielbergDramaMovies = moviesArray.filter(movie => movie.director === 'Steven Spielberg').map(movie => movie.genre).flat().filter(movie => movie === 'Drama').length
    const spielbergDramaMovies = moviesArray.filter(movie => movie.director === 'Steven Spielberg' && movie.genre.includes('Drama')).length
...
function scoresAverage(moviesArray) {
  if (!moviesArray.length || moviesArray.length === 0) return 0;

  const movieScoreSum = moviesArray.reduce((acc, currentVal) => {
    return acc + (currentVal.score ? currentVal.score : 0); //puts 0 if a movie doesnt have a score
  }, 0);

  const averageScore = +(movieScoreSum / moviesArray.length).toFixed(2);
...
function dramaMoviesScore(moviesArray) {
    ...
    const dramaMoviesScoreSum = dramaMovies.reduce((acc, currentVal) => {
        return acc + currentVal.score
    }, 0)
    
    const dramaMoviesAverageScore = +(dramaMoviesScoreSum / dramaMovies.length).toFixed(2);
    return dramaMoviesAverageScore
}
...
function orderByYear(moviesArray) {
  const moviesSortedByYear = [...moviesArray].sort((a, b) => {
    if (a.year === b.year) {
...
function orderAlphabetically(moviesArray) {
    const sortedMovieTitles = moviesArray.map(movie => movie.title).sort((a, b) => a.localeCompare(b))
    console.log(moviesArray)
    return sortedMovieTitles.length < 20 ? sortedMovieTitles : sortedMovieTitles.slice(0, 20)
}
...
function turnHoursToMinutes(moviesArray) {
  ...
  const arrayWithMinutes = moviesArray.map((movie) => {
    let minutes = 0
    if(movie.duration.includes('h')) {
        minutes = parseInt(movie.duration.split('h')[0]) * 60
    }
    if(movie.duration.includes('min')) {
        minutes += parseInt(movie.duration.split('min')[0].split('h')[1])
    }
